...
let characterFacingLeft = false; // Initialize to false, assuming character faces right by default
function drawCharacter() {
    const facingLeft = character.dx < 0;
    if (facingLeft && !characterFacingLeft) {
        characterFacingLeft = true;
    } else if (!facingLeft && characterFacingLeft && character.dx !== 0) {
        characterFacingLeft = false;
    }
    ctx.save();
    ctx.translate(character.x + character.width / 2, character.y + character.height / 2);
    ctx.scale(characterFacingLeft ? -1 : 1, 1);
    ctx.drawImage(characterImg, -character.width / 2, -character.height / 2, character.width, character.height);
    ctx.restore();
}
